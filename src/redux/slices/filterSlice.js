  ...
  async (_, { rejectWithValue, dispatch, getState }) => {
    ...
    try {
      ...
      const response = await axios(
        `/api/products/search?keywords=${searchValue}&offset=0&limit=${limit}`
      );
...
export const fetchMoreProductsBySearch = createAsyncThunk(
  "filter/fetchMoreProductsBySearch",
  async (itemsNumber, { rejectWithValue, dispatch, getState }) => {
    const limit = getState().products.limit;
    const searchValue = getState().filter.searchValue;
    try {
      dispatch(setLoading());
      const response = await axios(
        `/api/products/search?keywords=${searchValue}&offset=${itemsNumber}&limit=${limit}`
      );
...
export const { chooseCategory, chooseSortBy, changeSearchValue, resetFilters } =
  filterSlice.actions;
