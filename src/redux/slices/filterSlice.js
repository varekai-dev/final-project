...
export const fetchProductsBySort = createAsyncThunk('products/fetchProductsBySort', async (_, { rejectWithValue, dispatch, getState }) => {
	const sortBy = getState().filter.activeSortBy;
	const limit = getState().products.limit;
	try {
		dispatch(setLoading());
		const response = await axios(`/api/products?offset=0&limit=${limit}&sortBy=${sortBy.value}`);
...
export const fetchProductsByCategory = createAsyncThunk('filter/fetchProductsByCategory', async (_, { rejectWithValue, dispatch, getState }) => {
	...
	try {
		dispatch(setLoading());
		const response = await axios(`/api/categories${activeCategory ? `/${activeCategory}/products?limit=${limit}` : ''}`);
		dispatch(setProducts(response.data));
	} catch (error) {
		return rejectWithValue(error.response.data.error);
	} finally {
...
export const fetchMoreProductsByCategory = createAsyncThunk('products/fetchMoreProductsByCategory', async (itemsNumber, { rejectWithValue, getState, dispatch }) => {
	const activeCategory = getState().filter.activeCategory.id;
	const limit = getState().products.limit;
	try {
		dispatch(setLoading());
		const response = await axios(`/api/categories/${activeCategory}/products?offset=${itemsNumber}&limit=${limit}`);
...
export const fetchMoreProductsBySort = createAsyncThunk('products/fetchMoreProductsBySort', async (itemsNumber, { rejectWithValue, getState, dispatch }) => {
	const sortBy = getState().filter.activeSortBy;
	const limit = getState().products.limit;
	try {
		dispatch(setLoading());
		const response = await axios(`/api/products?offset=${itemsNumber}&limit=${limit}&sortBy=${sortBy.value}`);
		const data = response.data;
		const oldProductsList = getState().products.productsList;
		const newProductsList = [...oldProductsList, ...data];
		dispatch(setProducts(newProductsList));
...
export const { chooseCategory, chooseSortBy, changeSearchValue, resetSortBy, resetCategories } = filterSlice.actions;
